...
def test_9_r2_stop_is_canonical(market):
    """The sheet's Stop must be the canonical stop · SAME RUN ONLY.

    This compares a RENDERED artifact against a LIVE one. They agree only
    when both came from the same pipeline run. On 2026-09-11 India CI the
    sidecar was regenerated by that run while the workbook was the
    committed one, and the comparison reported a divergence in BOTH
    directions - VLO sheet 369.86 vs canonical 350.90, CRM sheet 226.01
    vs canonical 242.04. That is two different days' prices, not a
    delivery defect: a stop computed from Tuesday's close cannot equal
    one computed from Wednesday's.

    So the as-of is checked first. Same day, the stops must match
    exactly. Different day, this comparison has nothing to say and the
    renderer's fidelity to its own dataset is asserted by
    test_workbook_matches_the_lifecycle_dataset_exactly instead.
    """
    d = lifecycle(market)
    if not d:
        pytest.skip("no lifecycle dataset")
    # The canonical stop reaches the sheet THROUGH the lifecycle, which
    # is the one consumer of dynamic_risk_v2 and recomputes nothing. So
    # the renderer is checked against the dataset it actually rendered.
    #
    # Re-reading the sidecar live was fragile: a workflow regenerates it
    # mid-run, and a stop computed from a different session's close can
    # never equal one already rendered. That reported "divergence" in
    # BOTH directions (VLO 369.86 vs 350.90, CRM 226.01 vs 242.04) - two
    # days' prices, not a delivery defect.
    canon = {}
    for r in (d.get("current") or []):
        if str(r.get("engine")).strip() == "R2" and isinstance(
                r.get("stop"), (int, float)):
            canon[str(r["ticker"]).upper().split(".", 1)[0]] = round(
                float(r["stop"]), 4)
    wb = _wb(market)
    recs = _table(wb["CURRENT"], ["Ticker", "Engine", "Action"])
    wb.close()
