"""AEGIS USA · Factor Library runner (Sprint 7.5).

Reads Sprint 6.5 macro outputs from usa/reports/ and emits ONE row per factor.
"""
...
def main() -> int:
    ...
    print(f"[AEGIS USA · Factor Library · {now.isoformat(timespec='seconds')}]")

    result = build_factor_library(
        market="usa", reports_dir=REPORTS,
    ...
    register_model(_ROOT,
        ...
        notes="registered by usa/factor_library on " + date.today().isoformat(),
    )
    ...
    if not df.empty:
        ...
        try:
            if OUT_HISTORY.exists():
                ...
                keep = ~((existing["market"] == "usa") & (existing["asof"] == result.asof.isoformat()))
