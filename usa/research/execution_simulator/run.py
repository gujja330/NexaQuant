"""AEGIS USA · Execution Simulator runner (Sprint 7, USD)."""
...
OUT_LEDGER    = _USA / "reports" / "execution_ledger.parquet"
OUT_SUMMARY   = _USA / "reports" / "execution_summary.json"
OUT_CURVE     = _USA / "reports" / "equity_curve.parquet"
OUT_NARRATIVE = _USA / "reports" / "ai_execution_narrative.json"

PORTFOLIO_DIFF = _USA / "reports" / "portfolio_diff.json"
PORTFOLIO_V3   = _USA / "reports" / "portfolio_v3.json"
...
def main() -> int:
    ...
    print("=" * 70); print("  AEGIS USA · Execution Simulator v1 (Sprint 7, USD)"); print("=" * 70)

    if not PORTFOLIO_DIFF.exists():
        print("  FATAL: usa/reports/portfolio_diff.json missing"); return 1
    ...
    register_model(_ROOT,
        ...
        notes=f"registered by usa/execution_simulator on {now.date().isoformat()}",
    )
    model_stamp = stamp(_ROOT, model_id)

    engine = ExecutionEngine(
        repo_root=_ROOT, market="usa",
        starting_aum=float(cfg.get("starting_aum", 1_000_000)),
        min_slippage_bps=float(cfg.get("min_slippage_bps", 1.0)),
        liquidity_impact_bps=float(cfg.get("liquidity_impact_bps", 30.0)),
        vol_impact_bps=float(cfg.get("vol_impact_bps", 10.0)),
        commission_bps=float(cfg.get("commission_bps", 1.0)),
        max_daily_participation=float(cfg.get("max_daily_participation", 0.15)),
