...
def main():
    ...
    for i in range(LOOKBACK, len(closes) - CAD, CAD):
        ...
        for tag, n, acc in [("fix", 15, fix), ("dyn", n_dyn, dyn)]:
            sel = select_names(hist, n, sector_cap=2)
            if len(sel) < 2:
    ...
    F, D = summ(fix), summ(dyn)
    print("=" * 78)
    print("  AEGIS DYNAMIC POLICY — basket size by breadth+regime, walk-forward vs fixed N=15")
    print("=" * 78)
    print(f"  dynamic N ranged {min(nlist)}-{max(nlist)}, most-used {pd.Series(nlist).mode().iloc[0]}\n")
    print(f"  {'Policy':<22}{'CAGR':>8}{'Sharpe':>8}{'MaxDD':>8}{'Win%':>7}")
    print(f"  {'Fixed N=15':<22}{F['cagr']:>7.1f}%{F['sharpe']:>8.2f}{F['dd']:>7.1f}%{F['win']:>6.0f}%")
    print(f"  {'Dynamic N (breadth)':<22}{D['cagr']:>7.1f}%{D['sharpe']:>8.2f}{D['dd']:>7.1f}%{D['win']:>6.0f}%")
    edge = D["sharpe"] - F["sharpe"]
    print(f"\n  OOS Sharpe edge: {edge:+.2f}  ->  " + (
        "dynamic is at least as good — safe to adopt (and it adapts to the market)." if edge > -0.10
        else "dynamic underperforms; keep fixed unless you value adaptivity over the Sharpe cost."))
