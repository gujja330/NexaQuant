"""AEGIS India · Risk Engine runner (Sprint 4).

Reads:
  reports/recommendations_v3.json                  (Sprint 3 output)
  reports/market_intelligence_summary.json         (regime + VIX)
  reports/selected_features.json                   (Sprint 2.6)
  features/india/{latest}.parquet                  (Sprint 2.5)
  configs/risk_budget.yaml                         (operator-owned)
...
OUT_SIZED     = _ROOT / "reports" / "sized_positions.json"
OUT_REPORT    = _ROOT / "reports" / "risk_report.json"
OUT_NARRATIVE = _ROOT / "reports" / "ai_risk_narrative.json"

REC_PATH        = _ROOT / "reports" / "recommendations_v3.json"
...
def main() -> int:
    ...
    print("=" * 70); print("  AEGIS INDIA · Risk Engine v1 (Sprint 4)"); print("=" * 70)

    if not REC_PATH.exists():
        print("  FATAL: reports/recommendations_v3.json missing — run recommendation engine first"); return 1
    ...
    if not BUDGET_PATH.exists():
        print("  FATAL: configs/risk_budget.yaml missing"); return 1
    budget = _load_budget(BUDGET_PATH, "india")
    ...
    register_model(_ROOT,
        ...
        notes=f"registered by india/risk_engine on {now.date().isoformat()}",
    )
    model_stamp = stamp(_ROOT, model_id)

    engine = RiskEngine(
        repo_root=_ROOT, market="india", budget=budget,
    ...
    print(f"    port vol (ann): {report.portfolio_vol_annualised * 100:.2f}%")
    ...
    print(f"  wrote {OUT_SIZED.relative_to(_ROOT)}")
