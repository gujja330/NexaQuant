...
TRIGGERS = [
    ...
    ("News / EVENT database", [RAW / "event_database.parquet"],
     "EVENTS not generic sentiment: order wins, approvals, promoter buys, rating actions (timestamped)",
     ["FinBERT", "DeBERTa", "FinGPT", "Longformer", "event classifiers"]),
    ("Earnings & estimate revisions", [RAW / "earnings_revisions.parquet"],
     "earnings dates, surprise, 4-qtr trend, guidance, EPS/revenue revisions (IBES/Zacks-style)",
     ["LightGBM", "CatBoost", "LambdaMART (ranking)"]),
    ("Institutional flow history", [RAW / "institutional_flow.parquet"],
     "FII/DII/MF/ETF flow HISTORY (fii_dii.py collects forward only; no history yet)",
     ["LightGBM", "CatBoost", "gradient-boosted ranking"]),
