"""AEGIS India · Portfolio Engine runner (Sprint 5).

Reads:
  reports/sized_positions.json                     (Sprint 4)
  reports/market_intelligence_summary.json         (regime)
  configs/portfolio_config.yaml                    (operator-owned)
  reports/portfolio_state.json                     (prior day, may be absent)

Emits:
  reports/portfolio_v3.json                        (today's holdings + diff)
  reports/portfolio_diff.json                      (trade list to move prior → today)
  reports/portfolio_state.json                     (updated state — idempotent)
  reports/portfolio_state_history.jsonl            (append-only history)
  reports/ai_portfolio_narrative.json              (AI Portfolio Analyst)
"""
...
OUT_FULL      = _ROOT / "reports" / "portfolio_v3.json"
OUT_DIFF      = _ROOT / "reports" / "portfolio_diff.json"
OUT_NARRATIVE = _ROOT / "reports" / "ai_portfolio_narrative.json"

SIZED_PATH   = _ROOT / "reports" / "sized_positions.json"
...
def _ticker_sector_map(sized_positions: list[dict]) -> dict[str, str]:
    """Build ticker→sector from the sized positions (Sprint 4 embeds sector via feature snapshot).
    Empty if not populated; falls back to sized position `sector` field if present."""
    m: dict[str, str] = {}
...
def main() -> int:
    ...
    print("=" * 70); print("  AEGIS INDIA · Portfolio Engine v1 (Sprint 5)"); print("=" * 70)

    if not SIZED_PATH.exists():
        print("  FATAL: reports/sized_positions.json missing — run Risk Engine first"); return 1
    ...
    if not CONFIG_PATH.exists():
        print("  FATAL: configs/portfolio_config.yaml missing"); return 1
    ...
    register_model(_ROOT,
        ...
        notes=f"registered by india/portfolio_engine on {now.date().isoformat()}",
    )
    model_stamp = stamp(_ROOT, model_id)

    engine = PortfolioEngine(
        repo_root=_ROOT, market="india",
        target_n_positions=int(cfg.get("target_n_positions", 20)),
        min_position_size=float(cfg.get("min_position_size", 0.005)),
        cash_reserve_min=float(cfg.get("cash_reserve_min", 0.05)),
        cash_reserve_stress=float(cfg.get("cash_reserve_stress", 0.25)),
        rebalance_threshold_bps=int(cfg.get("rebalance_threshold_bps", 25)),
        regime=regime,
    ...
    # Emit portfolio_v3.json (comprehensive)
