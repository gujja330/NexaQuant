...
DEFINITIONS_ROWS = [
    ...
    ("", ""),
    ("── Exit History (90d) sheet (scope: REALIZED 90d · full canonical population) ──", ""),
    ("Population",
     "Every Registry-CLOSED opportunity in the last 90 calendar days · one canonical population shared with I20 · I28 · A23 validators · rotation artifacts (|pnl| ≤ 0.01%) excluded ONLY from summary statistics (still visible as rows for I20 lineage)"),
    ("Sort order",
     "Real trades first (rotation · signal_exit · target · stop_loss · most-recent first within group) · ORPHAN_AUTO_CLOSE rows LAST (system-cleanup · not operator trades) · composition banner shows the split explicitly"),
    ("Orphan-audit backup",
     "ORPHAN_AUTO_CLOSE rows are ALSO mirrored to reports/delivery/orphan_audit_{market}.jsonl as a defensive backup ledger · rows still appear in the sheet body so I20 (Registry-CLOSED ⊆ Exit History) passes"),
    ("Total exits",
    ...
    ("", ""),
    ("── Monthly Summary sheet (scope: REALIZED 90d · aggregated by month) ──", ""),
    ("Population",
     "Derived from Exit History (90d) · same rows · rotation artifacts (|pnl| ≤ 0.01%) excluded"),
    ("Row per month",
     "Month · N Exits · Wins · Losses · Total P&L % · Positive P&L % · Negative P&L % · Win Rate"),
    ("Separation contract (2026-08-28)",
     "Monthly summary lives on its OWN sheet · never as trailer rows inside Exit History body · validators never need trailer-skip logic"),
    ("", ""),
    ...
    ("", ""),
    ("── Missing-data semantics (HARD RULE 2026-08-28) ──", ""),
    ("Convention",
     "Any field the engine did not evaluate is rendered as '—' (em-dash) · NEVER as LOW · PENDING · 0 · or blank · because those are legitimate engine outputs for other populations"),
    ("Holding · no signal today",
     "Portfolio rows appended by Registry-ACTIVE completeness (Path A) show '—' for Urgency · Inv Quality · Investability · Sector when the today-signal engine did not fire for that ticker"),
    ("Why",
     "'LOW' urgency on a not-evaluated position was being misread as 'engine says LOW risk' · missing ≠ evaluated-and-safe"),
    ("", ""),
